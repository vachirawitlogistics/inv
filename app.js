...
async function doLogin() {
    ...
    if (!pwd) {
        return Swal.fire({ icon: 'warning', text: 'กรุณากรอกรหัสผ่าน', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 });
    }
    ...
    try {
        ...
        if (res.success) {
            ...
            Swal.fire({ icon: 'error', text: res.message, customClass: {popup: 'rounded-4'} }); 
            btn.innerHTML = originalBtnHTML;
            btn.disabled = false;
        }
    } catch(err) { 
    ...
    try {
        ...
        if (rawBillingData.length > 1) {
            for (let i = 1; i < rawBillingData.length; i++) {
                ...
                let status = r[15] ? r[15].toString().trim() : '';
        ...
        saveToCache('inv_cache_billingData', {
            raw: rawBillingData, totals: bkgTotalsGlobal, audit: auditData, ready: readyData
        });
...
function renderAuditTab() {
    ...
    for (let bkg of bkgKeys) {
        ...
        let dateStr = "";
        try { dateStr = new Date(group.date).toLocaleDateString('en-GB'); } catch(e) { dateStr = group.date; }
...
async function saveAuditBulk() {
    ...
    if(checkboxes.length === 0) {
        return Swal.fire({ icon: 'warning', title: 'ยังไม่ได้เลือกรายการ', text: 'กรุณาเลือกตู้ที่ต้องการอนุมัติ', customClass: {popup: 'rounded-4'} });
    }
    ...
    Swal.fire({ 
        ...
        showCancelButton: true, confirmButtonColor: '#4f46e5', confirmButtonText: 'ยืนยัน', cancelButtonText: 'ยกเลิก', customClass: {popup: 'rounded-4'}
    }).then(async result => {
        if(result.isConfirmed) {
            try { 
                const res = await callAPI('saveAuditData', { payload: payloadData }); 
                if(res.success) { 
                    Swal.fire({ icon: 'success', title: 'สำเร็จ!', text: res.message, customClass: {popup: 'rounded-4'} }); 
                    forceSyncAll();
...
function calcSummaryR() {
    ...
    document.querySelectorAll('.bkg-check-r:checked').forEach(cb => { 
        ...
        if(group) { 
            sInc += group.sumInc; sAdv += group.sumAdv; sTot += group.sumTot; 
            countBkg++; countCont += group.items.length; 
        } 
    });
    
    document.getElementById('selCountR').innerText = countBkg; 
    if (document.getElementById('selCountContR')) document.getElementById('selCountContR').innerText = countCont;
...
function revertBooking(bkg) {
    Swal.fire({ 
        ...
        showCancelButton: true, confirmButtonColor: '#f59e0b', confirmButtonText: 'ยืนยัน', cancelButtonText: 'ยกเลิก', customClass: {popup: 'rounded-4'}
    }).then(async res => {
        if(res.isConfirmed) {
            try { 
                const r = await callAPI('revertToAudit', { bookingNo: bkg }); 
                if(r.success) { 
                    Swal.fire({ icon: 'success', title: 'สำเร็จ', text: r.message, customClass: {popup: 'rounded-4'} }); 
                    forceSyncAll();
                } else { Swal.fire('เกิดข้อผิดพลาด', r.message, 'error'); }
            } catch(err) { Swal.fire('เกิดข้อผิดพลาด', err.message, 'error'); }
        }
    });
}
...
async function generateInvoiceBulk() {
    ...
    if(checkboxes.length === 0) {
        return Swal.fire({ icon: 'warning', title: 'ยังไม่ได้เลือกรายการ', text: 'กรุณาเลือก Booking เพื่อออก Invoice', customClass: {popup: 'rounded-4'} });
    }
        ...
        if(group) {
            group.items.forEach(obj => {
                ...
                tempPayloadForPDF.push({
                   rowIdx: obj.rowIdx, jobCustomer: r[4], customer: billToName, booking: r[6], date: r[0], plate: r[16] || '-',
                   cy: r[7] || '-', load: r[5] || '-', rtn: r[10] || '-', type: r[2] || '-', container: r[23] || '-', remark: r[14] || '',  
                   price: parseFloat(r[17]) || 0, adv1: parseFloat(r[18]) || 0, adv2: parseFloat(r[19]) || 0, adv6: parseFloat(r[24]) || 0,
                   ext1: parseFloat(r[20]) || 0, ext2: parseFloat(r[21]) || 0, ext3: parseFloat(r[22]) || 0, adv4: parseFloat(r[25]) || 0,
                   adv5: parseFloat(r[26]) || 0, adv9: parseFloat(r[27]) || 0, incTotal: obj.incTot, advTotal: obj.advTot, grandTotal: obj.grandTot
                }); 
    ...
    if(custSet.size > 1) {
        return Swal.fire({ icon: 'error', title: 'ไม่สามารถรวมบิลได้', text: 'ต้องเลือกลูกค้า (Bill To) เจ้าเดียวกันเพื่อรวมบิล 1 ใบ', customClass: {popup: 'rounded-4'} }); 
    }
...
async function confirmGeneratePDF() {
    ...
    if(!invDate || !finalBillTo) {
        return Swal.fire({ icon: 'warning', text: 'กรุณาระบุวันที่และชื่อลูกค้า', customClass: {popup: 'rounded-4'} });
    }
    ...
    try {
        const res = await callAPI('generateInvoicePDF', { 
            payload: tempPayloadForPDF, invoiceDate: invDate, billingUser: currentUser, isSplit: isSplit, customInvNo: customInvNo 
        });
        ...
        if(res.success) {
            ...
            Swal.fire({ 
                icon: 'success', title: 'สร้าง Invoice สำเร็จ!', html: htmlContent, showConfirmButton: false, showCloseButton: true, customClass: {popup: 'rounded-4'}, 
                didClose: () => { document.getElementById('tab-history').click(); forceSyncAll(); } 
            });
        } else { Swal.fire({ icon: 'error', title: 'เกิดข้อผิดพลาด', text: res.message, customClass: {popup: 'rounded-4'} }); }
    } catch(err) { Swal.fire('เกิดข้อผิดพลาด', err.message, 'error'); }
}
...
async function fetchFullHistoryInBackground() {
    try {
        ...
        if (fullData && !fullData.error) {
            ...
            populateHistoryDropdowns(rawHistoryData);
            if (document.getElementById('tab-history').classList.contains('active-history')) applyHistoryFilter();
        }
    } catch(e) {}
}
...
function sortHistory(col, keepDirection = false) {
    if(!keepDirection) { 
        ...
        else { histSortCol = col; histSortAsc = true; } 
    }
...
function renderHistoryTable() {
    ...
    if(filteredHistoryData.length === 0) { 
        ...
        if (limitVal !== 'ALL') dataToRender = filteredHistoryData.slice(0, parseInt(limitVal)); 

        dataToRender.forEach((r) => {
            let dateStr = "";
            try { dateStr = new Date(r.date).toLocaleDateString('en-GB'); } catch(e) { dateStr = r.date; }
                    ...
                    <td class="text-center pe-4">
                        <div class="btn-group dropstart">
                            <button type="button" class="btn btn-white border shadow-sm btn-sm fw-bold dropdown-toggle dropdown-toggle-split rounded-pill px-3 text-secondary" data-bs-toggle="dropdown" aria-expanded="false">
                                <span class="visually-hidden">เปิดเมนู</span><i class="bi bi-three-dots"></i>
                            </button>
...
async function viewInvoiceDetails(invNo) {
    try {
        ...
        if (res && res.error) return Swal.fire('เกิดข้อผิดพลาด', res.error, 'error');
        ...
        Swal.fire({ 
            ...
            width: '650px', showCloseButton: true, showConfirmButton: false, customClass: {popup:'rounded-4', title:'mb-0'} 
        });
    } catch(err) { Swal.fire('เกิดข้อผิดพลาด', err.message, 'error'); }
}
...
async function openEditInvoiceModal(invoiceNo) {
    try {
        ...
        if(!res.success) return Swal.fire('เกิดข้อผิดพลาด', res.message, 'error');
        
        document.getElementById('editInvNo').value = res.invoiceNo;
        let dStr = ""; 
        if(res.invoiceDate) { 
            let d = new Date(res.invoiceDate); 
            if(!isNaN(d)) dStr = d.getFullYear() + "-" + String(d.getMonth()+1).padStart(2,'0') + "-" + String(d.getDate()).padStart(2,'0'); 
        }
            ...
            html += `
                <div class="card shadow-sm border border-light mb-3 edit-item-card rounded-3 overflow-hidden" data-booking="${itm.booking}" data-container="${itm.container}">
                    <div class="card-header bg-white border-bottom pt-2 pb-1">
                        <h6 class="fw-bold text-dark mb-0 fs-6"><i class="bi bi-box-seam me-2 text-primary"></i>${itm.container} <span class="badge bg-light text-secondary border fw-medium ms-2">BKG: ${itm.booking}</span></h6>
                    </div>
                    <div class="card-body bg-light p-2 px-3">
                        <div class="row g-3">
                            <div class="col-md-6 border-end border-light">
                                ...
                                <div class="row g-2">
                                    <div class="col-6"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ราคาเที่ยว</label><input type="number" class="form-control form-control-sm bg-white e-price" value="${itm.price}"></div>
                                    <div class="col-6"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ต่อระยะ</label><input type="number" class="form-control form-control-sm bg-white e-ext1" value="${itm.ext1}"></div>
                                    <div class="col-6"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ค้างหาง</label><input type="number" class="form-control form-control-sm bg-white e-ext2" value="${itm.ext2}"></div>
                                    <div class="col-6"><label class="small text-muted mb-0" style="font-size: 0.75rem;">เสียเวลา</label><input type="number" class="form-control form-control-sm bg-white e-ext3" value="${itm.ext3}"></div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <p class="text-danger fw-bold small mb-2"><i class="bi bi-arrow-down-right-circle me-1"></i>สำรองจ่าย (Advance)</p>
                                <div class="row g-2">
                                    <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">รับตู้</label><input type="number" class="form-control form-control-sm bg-white e-adv1" value="${itm.adv1}"></div>
                                    <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">คืนตู้</label><input type="number" class="form-control form-control-sm bg-white e-adv2" value="${itm.adv2}"></div>
                                    <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ผ่านท่า</label><input type="number" class="form-control form-control-sm bg-white e-adv3" value="${itm.adv3}"></div>
                                    <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ซ่อมตู้</label><input type="number" class="form-control form-control-sm bg-white e-adv4" value="${itm.adv4}"></div>
                                    <div class="col-4"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ล้างตู้</label><input type="number" class="form-control form-control-sm bg-white e-adv5" value="${itm.adv5}"></div>
                                    <div class="col-4"><label class="small text-primary fw-bold mb-0" style="font-size: 0.75rem;">ค่าชอ</label><input type="number" class="form-control form-control-sm border-primary bg-primary bg-opacity-10 e-adv6" value="${itm.adv6}"></div>
                                    <div class="col-12 m-0 mt-1"><hr class="m-0 opacity-25"></div>
                                    <div class="col-8 mt-1"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ยอดอื่น 1 (ชื่อ)</label><input type="text" class="form-control form-control-sm bg-white e-extn1" value="${itm.extn1}"></div>
                                    <div class="col-4 mt-1"><label class="small text-muted mb-0" style="font-size: 0.75rem;">ยอด</label><input type="number" class="form-control form-control-sm bg-white e-extv1" value="${itm.extv1}"></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
function confirmEditInvoice() {
    ...
    if(!invDate || !customer) return Swal.fire({ icon: 'warning', text: 'กรุณาระบุวันที่และชื่อลูกค้า', customClass: {popup: 'rounded-4'} });
    
    let updatedItems = [];
    document.querySelectorAll('.edit-item-card').forEach(card => {
        updatedItems.push({
            booking: card.getAttribute('data-booking'), container: card.getAttribute('data-container'), 
            price: parseFloat(card.querySelector('.e-price').value)||0, ext1: parseFloat(card.querySelector('.e-ext1').value)||0, 
            ext2: parseFloat(card.querySelector('.e-ext2').value)||0, ext3: parseFloat(card.querySelector('.e-ext3').value)||0,
            adv1: parseFloat(card.querySelector('.e-adv1').value)||0, adv2: parseFloat(card.querySelector('.e-adv2').value)||0, 
            adv3: parseFloat(card.querySelector('.e-adv3').value)||0, adv4: parseFloat(card.querySelector('.e-adv4').value)||0, 
            adv5: parseFloat(card.querySelector('.e-adv5').value)||0, adv6: parseFloat(card.querySelector('.e-adv6').value)||0,
            extn1: card.querySelector('.e-extn1').value.trim(), extv1: parseFloat(card.querySelector('.e-extv1').value)||0, 
            extn2: card.querySelector('.e-extn2') ? card.querySelector('.e-extn2').value.trim() : '', extv2: 0
        });
    });

    Swal.fire({ 
        title: 'ยืนยันการอัปเดต?', text: "สร้าง PDF ใบใหม่ทับของเดิม", icon: 'warning', showCancelButton: true, 
        confirmButtonColor: '#1e293b', confirmButtonText: 'ยืนยันอัปเดต', cancelButtonText: 'ยกเลิก', customClass:{popup:'rounded-4'}
    }).then(async (result) => {
        if (result.isConfirmed) {
            try {
                const res = await callAPI('saveEditedInvoice', { invoiceNoStr: invNo, invoiceDate: invDate, customerName: customer, isSplit: isSplit, items: updatedItems, billingUser: currentUser });
                if(res.success) {
                    ...
                    Swal.fire({ icon: 'success', title: 'อัปเดตสำเร็จ!', html: htmlContent, showConfirmButton: false, showCloseButton: true, customClass:{popup:'rounded-4'}, didClose: () => { forceSyncAll(); } });
                } else { Swal.fire({ icon: 'error', text: res.message, customClass:{popup:'rounded-4'} }); }
            } catch(err) { Swal.fire('เกิดข้อผิดพลาด', err.message, 'error'); }
        }
    });
}
...
function promptRollback(invoiceNo) {
   Swal.fire({ 
       title: 'ยกเลิกบิล (Rollback)?', html: `ต้องการยกเลิก Invoice <b class="text-danger">${invoiceNo}</b> และส่งกลับไปหน้าเตรียมวางบิลหรือไม่`, icon: 'warning', 
       showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'ยืนยันยกเลิกบิล', cancelButtonText: 'ปิด', customClass:{popup:'rounded-4'}
   }).then(async res => {
      if(res.isConfirmed) {
         try { 
             const r = await callAPI('rollbackInvoice', { invoiceNo: invoiceNo }); 
             if(r.success) { Swal.fire({ icon: 'success', title: 'สำเร็จ', text: r.message, customClass: {popup:'rounded-4'} }); forceSyncAll(); } 
             else { Swal.fire('เกิดข้อผิดพลาด', r.message, 'error'); } 
         } catch(err) { Swal.fire('เกิดข้อผิดพลาด', err.message, 'error'); }
      }
   });
...
function promptRollbackReceipt(invoiceNo, docNoToDel, docTypeToDel) {
   ...
   Swal.fire({ 
       ...
       showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: `ยืนยันลบ`, cancelButtonText: 'ยกเลิก', customClass:{popup:'rounded-4'}
   }).then(async res => {
      if(res.isConfirmed) {
         try { 
             const r = await callAPI('rollbackReceipt', { invoiceNo: invoiceNo, docType: docTypeToDel }); 
             if(r.success) { Swal.fire({ icon: 'success', title: 'ลบสำเร็จ', text: r.message, customClass:{popup:'rounded-4'} }); forceSyncAll(); } 
             else { Swal.fire('เกิดข้อผิดพลาด', r.message, 'error'); } 
         } catch(err) { Swal.fire('เกิดข้อผิดพลาด', err.message, 'error'); }
      }
   });
...
async function printPDF(docNo) {
   try {
       ...
       if(res.success) { 
           Swal.fire({ title: `<i class="bi bi-file-earmark-pdf text-primary fs-1"></i>`, html: `<h4 class="fw-bold mb-3">${docNo}</h4><a href="${res.url}" target="_blank" class="btn btn-primary btn-lg rounded-pill px-5 fw-bold shadow" onclick="Swal.close()">เปิดเอกสาร</a>`, showConfirmButton: false, showCloseButton: true, customClass:{popup:'rounded-4'} }); 
       } else { Swal.fire({ icon: 'error', text: res.message, customClass:{popup:'rounded-4'} }); }
   } catch(err) { Swal.fire('เกิดข้อผิดพลาด', err.message, 'error'); }
}
...
function openReceiptModal(invNo, docType) {
    ...
    if(document.getElementById('recModeDetail')) document.getElementById('recModeDetail').checked = true; 
    if(document.getElementById('recShowDueDate')) document.getElementById('recShowDueDate').checked = true;
    if(btn) btn.disabled = false;
    
    if(document.getElementById('receiptSettingModal')) new bootstrap.Modal(document.getElementById('receiptSettingModal')).show();
}
...
async function confirmGenerateReceipt() {
    ...
    if(!recDate) return Swal.fire({ icon: 'warning', text: 'กรุณาระบุวันที่', customClass: {popup: 'rounded-4'} });
    
    let btnConf = document.getElementById('btnConfirmReceipt');
    if(btnConf) btnConf.disabled = true;

    try {
        const res = await callAPI('generateReceiptPDF', { invoiceNo: currentReceiptInvNo, docDate: recDate, layoutMode: layoutMode, billingUser: currentUser, docType: currentDocType, showDueDate: showDueDate });
        if(btnConf) btnConf.disabled = false;
        
        if(res.success) {
            if(document.getElementById('receiptSettingModal')) bootstrap.Modal.getInstance(document.getElementById('receiptSettingModal')).hide();
            ...
            Swal.fire({ 
                icon: 'success', title: `สร้าง${docName}สำเร็จ!`, html: `<a href="${res.pdfUrl}" target="_blank" class="btn ${btnClass} btn-lg rounded-pill mt-3 px-5 fw-bold shadow-sm" onclick="Swal.close()"><i class="bi bi-file-earmark-pdf-fill me-2"></i> เปิดดูเอกสาร</a>`, 
                showConfirmButton: false, showCloseButton: true, customClass: {popup: 'rounded-4'}, didClose: () => { forceSyncAll(); } 
            });
        } else { Swal.fire({ icon: 'error', text: res.message, customClass: {popup: 'rounded-4'} }); }
    } catch(err) {
        if(btnConf) btnConf.disabled = false; 
